...
import './DoctorMapPage.css';
import Navbar from '../components/Navbar/NavBar';
import Squares from '../components/squares/Squares';

// Fix for default marker icon issue in React-Leaflet
// We need to reset the icon paths for Leaflet's default markers
...
const DoctorMapPage = () => {
  ...
  useEffect(() => {
    const uniqueSpecializations = [...new Set(dummyDoctors.map(doctor => doctor.specialization))];
  ...
  useEffect(() => {
    if (selectedSpecialization) {
      setFilteredDoctors(dummyDoctors.filter(doctor => doctor.specialization === selectedSpecialization));
    } else {
      setFilteredDoctors(dummyDoctors);
    }
  ...
  // Find nearby doctors when user location changes
  useEffect(() => {
    if (userLocation) {
      const doctorsWithDistance = dummyDoctors.map(doctor => ({
        ...doctor,
      ...
      // Sort by distance and get the 10 closest doctors
      const closest = doctorsWithDistance
        .sort((a, b) => a.distance - b.distance)
        .slice(0, 10);
      
      setNearbyDoctors(closest);
      setFilteredDoctors(closest);
    }
  }, [userLocation]);
  ...
  const handleFilterChange = (e) => {
    ...
    // Reset nearby doctors filter when changing specialization
    if (userLocation && e.target.value === '') {
      // If no specialization is selected but we have user location, show nearby doctors
      setFilteredDoctors(nearbyDoctors);
    }
  };
  
  // Handle find doctors near me button click
...
hoverFillColor='#222'
/>
      <div className="doctor-map-header">
        <h1>Find Doctors Near You</h1>
        <p className="doctor-map-subtitle">Explore our network of healthcare professionals on the map</p>
      </div>
      ...
      <div className="filter-section">
        <div className="filter-controls">
          ...
          <button className="find-near-me-btn" onClick={handleFindNearMe}>
            ...
            >
              <Popup>
                <div className="doctor-popup">
                  ...
                  <p><strong>Address:</strong> {doctor.officeLocation.address}</p>
      ...
      <div className="doctor-cards-container">
        <h2 className="doctor-cards-title">Available Doctors</h2>
